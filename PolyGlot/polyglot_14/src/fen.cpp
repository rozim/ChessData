...
bool board_from_fen(board_t * board, const char string[]) {
   ...
   for (rank = 7; rank >= 0; rank--) {
      ...
      for (file = 0; file < 8;) {
         ...
         if (c >= '1' && c <= '8') { // empty square(s)
            ...
            len = c - '0';
            if (file + len > 8) my_fatal("board_from_fen(): bad FEN/a (pos=%d)\n",pos);
            ...
            if (piece == PieceNone256) my_fatal("board_from_fen(): bad FEN/b (pos=%d)\n",pos);
      ...
      if (rank > 0) {
         if (c != '/') my_fatal("board_from_fen(): bad FEN/c (pos=%d)\n",pos);
         c = string[++pos];
     }
   }
   ...
   if (c != ' ') my_fatal("board_from_fen(): bad FEN/d (pos=%d)\n",pos);
   c = string[++pos];
   ...
   case 'b':
      ...
      break;
   default:
      my_fatal("board_from_fen(): bad FEN/e (pos=%d)\n",pos);
      break;
   }
   ...
   if (c != ' ') my_fatal("board_from_fen(): bad FEN/f (pos=%d)\n",pos);
   c = string[++pos];
   ...
   if (c == '-') { // no castling rights
      ...
      do {
         ...
         if (false) {
            ...
            my_fatal("board_from_fen(): bad FEN/g (pos=%d)\n",pos);
         }
   ...
   if (c != ' ') my_fatal("board_from_fen(): bad FEN/h (pos=%d)\n",pos);
   c = string[++pos];
   ...
   if (c == '-') { // no en-passant
      ...
      if (c < 'a' || c > 'h') my_fatal("board_from_fen(): bad FEN/i (pos=%d)\n",pos);
      file = file_from_char(c);
      c = string[++pos];

      if (c < '1' || c > '8') my_fatal("board_from_fen(): bad FEN/j (pos=%d)\n",pos);
   ...
   if (c != ' ') {
      ...
      my_fatal("board_from_fen(): bad FEN/k (pos=%d)\n",pos);
   }
   c = string[++pos];

   if (!isdigit(c)) {
      if (!Strict) goto update;
      my_fatal("board_from_fen(): bad FEN/l (pos=%d)\n",pos);
   }
   ...
   if (c != ' ') {
      if (!Strict) goto update;
      my_fatal("board_from_fen(): bad FEN/m (pos=%d)\n",pos);
   }
   c = string[++pos];

   if (!isdigit(c)) {
      if (!Strict) goto update;
      my_fatal("board_from_fen(): bad FEN/n (pos=%d)\n",pos);
   }
