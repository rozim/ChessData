   ...
   while (true) {
      ...
      if (false) {
         ...
         if (pgn->token_type != TOKEN_SYMBOL) {
           ...
           my_fatal("pgn_next_move(): malformed move at line %d, column %d [%s]\n",pgn->token_line,pgn->token_column,
                    pgn->token_string);
         }
   ...
   if (false) {
      ...
      do {
         ...
         if (pgn->token_length >= PGN_STRING_SIZE-1) {
           pgn->token_string[PGN_STRING_SIZE-1] = '\0';
           my_fatal("pgn_read_token(): symbol too long at line %d, column %d [%s]\n",pgn->char_line,pgn->char_column,
                    pgn->token_string);
         }
...
static void pgn_skip_blanks(pgn_t * pgn) {
   ...
   while (true) {
      ...
      if (false) {
                 ...
                 pgn->char_hack == 26) { // hack for enormous.pgn - has ^Z's 
